# Brain-Shield — Public Proof Pack
  ...
  **A unified anti-rug shield for Solana memecoin traders, built on Jito ShredStream.**
  ...
  > 💬 **JIP-Ideas thread** :
  [forum.jito.network/t/brain-shield-a-defensive-use-case-for-shredstream](https://forum.jito.network/t/brain-shield-a-defensive-use-case-for-shredstream-opt-in-trade-insurance-for-memecoin-holders/938) —
  ...
  | # | Slot | Solscan TX |
  |---|---|---|
  | 1 | 417 822 435 | [5aFJw4rN…ftgrM](https://solscan.io/tx/5aFJw4rNWMrhUQmw1NzrduC6Fnvaiy8ZwXBx7MUxC6ZnUxyttdt2qEDygBoYdkuEB8K88fQ8C1a5VepgCrKftgrM) |
  | 2 | 417 829 059 | [4VG6E6X9…DTY6u](https://solscan.io/tx/4VG6E6X9cgjcRfeeD1HM3Ub8N8bhjiXSMZ4ckr1jYxyzC2NVJEjSRS1aE4TLnAwPfgaZBNcHQj9oU9jgGwuDTY6u) |
  | 3 | 417 834 959 | [5gtXhkQn…sdGd4](https://solscan.io/tx/5gtXhkQnLRtT5Pvu1eDM22Gscwx7tfsipw3qG82KxQywCCBzSNj6CHHRSBJAVeZnVT5hbhGJ8NKakLK25YjsdGd4) |
  | 4 | 417 848 320 | [4frBTsck…ME1AM](https://solscan.io/tx/4frBTsckAT55qPv6vC6Xjp3sX6mHPB4ZYV7UL1WMwdu3N7M8ZfREY4vt7VPZ29hQjzLxBURnQYDLi3fpdDJME1AM) |
  | 5 | 417 878 187 | [PkFYg54W…issk](https://solscan.io/tx/PkFYg54WsNCpW3CGGPy9LNvm4XdrDnJoSrozvjX36WCqqjaaYxDzHagN61q91ay7FiyuUM528wSyXw7APFPissk) |

  Full list of 10 representative samples (with mint / pool / signer fields) in [`SAMPLES.md`](./SAMPLES.md).
  ...
  **Detection stack today** : gradient-boosted models (LightGBM, AUC 0.80–0.88
  cross-validated) running on ShredStream-derived features. A reinforcement
  learning policy is under training on a 1.3M-token / 154M-event dataset to
  replace the heuristic scoring once stability is validated.
  ...
  ## How to verify this proof pack

  Anyone can independently reproduce the on-chain side of the benchmark in
  under 5 minutes :

  1. **Pick any Solscan link** from the table above (or from `SAMPLES.md`).
  2. **Open the transaction** on Solscan. Confirm the `Withdraw` instruction
     targets program `pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA`
     (Pump.fun AMM) and removes WSOL + the quote token from the listed pool.
  3. **Cross-check the fields** (pool, signer, slot) against the values
  ...
  Brain-Shield is built by **Walid** — an independent Solana developer based
  in France, focused on memecoin market microstructure and anti-rug
  infrastructure since 2023.

  - 3 years building low-latency Solana trading systems on ShredStream
  - Author of the 3-layer anti-rug detection stack (LP withdrawal,
    coordinated dump, cartel operations)
  - Indexed 960+ active cartels from historical on-chain data
  - Currently training a reinforcement learning policy on a 1.3M-token /
    154M-event dataset for cartel detection

  **Contact**

  - Email : wrahali568@gmail.com
  - Discord : `wicode06`
  - Jito forum thread : [forum.jito.network/t/.../938](https://forum.jito.network/t/brain-shield-a-defensive-use-case-for-shredstream-opt-in-trade-insurance-for-memecoin-holders/938)

  For grant evaluators, code reviewers, or anyone wanting to discuss the
  implementation under NDA — please reach out by email or Discord.
